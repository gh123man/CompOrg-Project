...
eval:
	...
	lb	$s2, 0($s0)				#save current value
	
	bne	$s2, $zero, eval_not_found_fixed	#if the current location is not 0 save value
...
eval_not_found_fixed:
	...
	#for each possible piece place
	#is valid place? call eval on next
	#not valid? try again
	#none valid? return false
	
	$li	$s4, 1					#s4 is our counter for the loop (cant use 0)
	
eval_loop:
	
	beq	$s4, $s2, eval_loop_done
	
	
	jal	place_and_validate			#place and validate
	beq	$v0, $zero, eval_loop_bottom		#branch if bad place
	
	bne	$s1, $s3, not_last_place
	
	#was last place
	#was good place so return ture
	li	$v0, 1
	j	eval_end
